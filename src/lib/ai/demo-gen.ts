...
### 技术要求
- 单个 HTML 文件，所有 CSS 和 JavaScript 必须内联，**不允许任何外部资源**（无 CDN、无外部 URL、无 img src 外链）
- 用原生 CSS（不用 Tailwind CDN），自己写样式
- 用原生 JavaScript（无框架）
- 响应式，针对 ${frameDesc}
...
export async function generateDemo(sessionId: string, modelId?: string): Promise<void> {
  ...
  try {
    ...
    // Single-step: generate PRD inline and HTML together (faster, fewer tokens)
    const prd = await generatePRD(plan, locale, client, model);
    // Store PRD but don't await demo separately — pass plan directly for speed
